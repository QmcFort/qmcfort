...
function run_test() {
  cp qmcfort_in_int qmcfort_in
  qmcfort_run 1 4
  cp qmcfort_out qmcfort_out_int
  cp qmcfort_log qmcfort_log_int
  cp timing timing_int
  ...
  rm -f values.txt
  touch values.txt
  hf_search "qmcfort_out_fs" "values.txt"
...
function update_test() {
  ...
  rsync -a $result/qmcfort_out_int $source/ref_qmcfort_out_int.txt
  rsync -a $result/qmcfort_out_hf $source/ref_qmcfort_out_hf.txt
  rsync -a $result/qmcfort_out_fs $source/ref_qmcfort_out_fs.txt
  rsync -a $result/timing_int $source/ref_timing_int.txt
  rsync -a $result/timing_hf $source/ref_timing_hf.txt
  rsync -a $result/timing_fs $source/ref_timing_fs.txt
}