...
function run_test() {
  ...
  cp qmcfort_in_noci qmcfort_in
  qmcfort_run 4 1
  cp qmcfort_out qmcfort_out_noci
  cp qmcfort_log qmcfort_log_noci
  cp timing timing_noci

  #extract values for comparison
  rm -f values.txt
  touch values.txt
  noci_afqmc_search "qmcfort_out_noci" "values.txt"
...
function update_test() {
  ...
  rsync -a $result/qmcfort_out_noci $source/ref_qmcfort_out_noci.txt
  rsync -a $result/timing_hf $source/ref_timing_hf.txt
  rsync -a $result/timing_noci $source/ref_timing_noci.txt
}