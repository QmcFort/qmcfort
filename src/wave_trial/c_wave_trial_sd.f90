...
private
public :: CWaveTrialSD, init_c_wave_trial_sd

type, extends(WaveTrial) :: CWaveTrialSD
  complex(wp), allocatable :: coeff(:,:,:)
  complex(wp), allocatable :: coeff_occ(:,:)
  complex(wp), allocatable :: h1phi(:,:)
  complex(wp), allocatable :: h1phi_s(:,:)
  complex(wp), allocatable :: h2phi_h(:,:)
  complex(wp), allocatable :: h2phi_s(:,:)
  complex(wp), allocatable :: h2phi_x_chol(:,:)
  complex(wp), allocatable :: h2phi_x_int(:,:,:)
  complex(wp), allocatable :: h2phi_full(:,:,:,:)
  complex(wp), allocatable :: hselfphi(:,:,:)
  complex(wp), allocatable :: hselfphi_gres(:,:,:,:)
contains
...
end type CWaveTrialSD

interface CWaveTrialSD
  module procedure finit_c_wave_trial_sd
end interface CWaveTrialSD

contains

!******************************************************************************** 
!
! Initialization of the CWaveTrialSD object
!
!******************************************************************************** 
subroutine init_c_wave_trial_sd(wtdes, ham, coeff, self)
  type(WaveTrialDes), intent(in)     :: wtdes
  type(Hamilton), target, intent(in) :: ham
  complex(wp), intent(in)            :: coeff(:,:,:)
  type(CWaveTrialSD), intent(out)    :: self
...
end subroutine init_c_wave_trial_sd


!******************************************************************************** 
!
! CWaveTrialSD constructor
!
!******************************************************************************** 
function finit_c_wave_trial_sd(wtdes, ham, coeff) result(self)
  type(WaveTrialDes), intent(in)     :: wtdes
  type(Hamilton), target, intent(in) :: ham
  complex(wp), intent(in)            :: coeff(:,:,:)
  type(CWaveTrialSD)                 :: self

  call init_c_wave_trial_sd(wtdes, ham, coeff, self)
end function finit_c_wave_trial_sd
...
subroutine assign_wave_trial_sd(to, from)
  class(CWaveTrialSD), intent(inout) :: to
  class(WaveTrial), intent(in)       :: from

  select type (from)
    type is (CWaveTrialSD)
...
!******************************************************************************** 
!
! Setup CWaveTrialSD object for the energy evaluation
!
!******************************************************************************** 
subroutine setup_wave_trial_sd(self, lself)
  class(CWaveTrialSD), intent(inout) :: self
...
!
!******************************************************************************** 
subroutine setup_h2_sd(self, h2, lself)
  class(CWaveTrialSD), intent(inout) :: self
  real(wp), intent(in)               :: h2(:,:,:)
  logical, intent(in)                :: lself
  
  call get_h2phi(self%coeff, h2, self%h2phi_full)

  if (lself) then
    call get_hselfphi(self%coeff, h2, self%hselfphi_gres)
...
subroutine report_wave_trial_sd(self, comm, fh)
  ...
  if (comm%mpirank == 0) then
    ...
    write(fh%funit,*)   "-------------------------------" 
    write(fh%funit,100) "trial wave function type        ", "CWaveTrialSD"
...
function sizeof_wave_trial_sd(self) result(mem)
  class(CWaveTrialSD), intent(in) :: self
  real(dp)                        :: mem
...
subroutine trial_energy_sd(self, e1, lgmean, eh, ex, eself)
  ...
  call self%local_energy_gres(self%coeff_occ, e1, lgmean, eh, ex, eself)
...
subroutine trial_h2_gres_mean_sd(self, lgmean)
  ...
  call self%h2_gres_mean(self%coeff_occ, lgmean, eh)
...
subroutine local_energy_batch_sd(self, coeff, e1, lgmean, eh, ex, eself, lsamplex, lself, en)
  ...
  if (lself_) call h2_gres_mean_no_batch(self%ham%des, self%h2phi_s, coeff_bi, eself_)
  ...
  do w = 1, nw
    ...
    if (lself_) eself(w) = sum(eself_(:,w))
  end do
